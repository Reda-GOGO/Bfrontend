    ...
    <View style={styles.invoiceData.container}>
      ...
      <View style={styles.invoiceData.secondRow}>
        <View style={styles.invoiceData.firstCol}>
          <Text style={{ fontSize: 9 }}>Bon de Livraison {order.id}</Text>
        </View>
...
function InvoiceDeliver({ order }: { order: Order }) {
  return (
    <View
      style={{
        display: "flex",
    ...
    >
      <DeliverBox label={"Cachet magasinier"} />
      <DeliverBox label={"Caché par"} />
      <DeliverBox label={"Client Chargé"} />
    </View>
  );
}

function DeliverBox({ label, value }) {
  return (
    <View>
      <Text> {label}</Text>
      <View
...
export const DeliverPDF = ({ order, logoUrl }: { order: Order }) => (
  <Document>
