...
import type { Collection } from "@/types"; // adjust path if needed
...
export default function MediaCard({ collection, setCollection }: Props) {
  ...
  const formatImageUrl = (image: string | null) => {
    ...
    if (isImageStored) {
      ...
      return `${import.meta.env.VITE_API_URL + collection.image}`;
    }
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCollection((prev) => ({ ...prev, image: preview, imageFile: file }));
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Select an image for your collection</CardDescription>
      </CardHeader>
